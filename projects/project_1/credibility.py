...
def fetch_page(url: str) -> Optional[str]:
    """
    Retrieve the HTML content of a webpage for credibility analysis.

    The original baseline only examines the URL string. This function allows
    the scorer to inspect the actual page while using a short timeout so an
    unreachable or slow website does not cause the application to hang.
    If the page cannot be retrieved, None is returned and URL rules still work.
    """
    try:
        response = requests.get(
            url,
            timeout=5,
            headers={"User-Agent": "Mozilla/5.0 CredibilityScorer/1.0"},
        )
...
def page_based_signals(html: str) -> List[Signal]:
    """
    Inspect retrieved HTML for evidence that can support credibility scoring.

    These features do not prove that a page is trustworthy. Instead, they add
    small signals for transparency and sourcing, such as named authors,
    publication dates, references, and scholarly citation metadata.
    """
    signals: List[Signal] = []
    ...
    # HTML tags and metadata are not consistently capitalized across websites.
    # Converting the page to lowercase lets us search for common patterns
    # without separately checking "author", "Author", "AUTHOR", and so on.
    page = html.lower()

    # Author metadata provides evidence that responsibility for the content
    # is attributed to a named creator. This is only a small positive signal
    # because simply having an author does not guarantee accurate information.
    author_patterns = [
        'name="author"',
        "name='author'",
    ...
    if any(pattern in page for pattern in author_patterns):
        signals.append(
            Signal("page_author", 0.03, "page includes author metadata")
        )

    # Publication metadata helps establish when an article was created or
    # updated. Dating a source improves transparency, but a publication date
    # alone is not evidence that the claims on the page are correct.
    date_patterns = [
    ...
    if any(pattern in page for pattern in date_patterns):
        signals.append(
            Signal("page_date", 0.02, "page includes publication-date metadata")
        )

    # Citation metadata is commonly used by scholarly publishing systems.
    # A DOI or citation title suggests a formally identified publication,
    # so this receives more weight than ordinary author or date metadata.
    scholarly_patterns = [
        'name="citation_doi"',
        'name="citation_title"',
        'name="dc.identifier"',
    ]
    if any(pattern in page for pattern in scholarly_patterns):
        signals.append(
            Signal("scholarly_metadata", 0.06, "page includes scholarly citation metadata")
        )

    # Reference sections can indicate that a page identifies supporting
    # sources. This remains a modest signal because references themselves
    # can still be weak, irrelevant, or incorrectly interpreted.
    reference_patterns = [
        ">references<",
        ">references:",
        'id="references"',
    ...
    if any(pattern in page for pattern in reference_patterns):
        signals.append(
            Signal("references", 0.03, "page appears to include references")
        )

    # Preprints can contain authors, citations, DOIs, and other scholarly
    # features while still lacking formal peer review. Detecting an explicit
    # preprint label prevents scholarly appearance alone from being treated
    # as equivalent to a peer-reviewed publication.
    # A page may mention the word "preprint" simply because a peer-reviewed
    # article cites one in its references. Use more specific phrases that
    # identify the current document as a preprint to reduce false positives.
    preprint_patterns = [
        "this preprint",
        "this manuscript is a preprint",
        "this article is a preprint",
        "preprint has not been peer reviewed",
        "preprint has not yet been peer reviewed",
    ]

    if any(pattern in page for pattern in preprint_patterns):
        signals.append(
            Signal(
                "preprint",
                -0.12,
                "page identifies the publication as a preprint",
            )
        )
...
def score_url(url: str, use_llm: Optional[bool] = None) -> Dict[str, Any]:
    ...
    # Layer 1 always runs.
    # Start with the original URL-based credibility signals, then attempt to
    # inspect the webpage itself. Page-level evidence supplements the baseline
    # rather than replacing it, so the scorer still works when a page is
    # unavailable, blocked, or returns an error.
    signals = rule_based_signals(url)

    page = fetch_page(url)
    if page is not None:
        signals.extend(page_based_signals(page))

    # Some preprint repositories may not expose an explicit "preprint" phrase
    # in the fetched page HTML, so use the repository domain as a fallback.
    # Avoid adding the penalty twice if page-based detection already found it.
    hostname = (parsed.hostname or "").lower()
    if hostname.startswith("www."):
        hostname = hostname[4:]

    if (
        hostname in {"arxiv.org", "biorxiv.org"}
        and not any(signal.name == "preprint" for signal in signals)
    ):
        signals.append(
            Signal(
                "preprint",
                -0.12,
                "source is a recognized preprint repository",
            )
        ) 
